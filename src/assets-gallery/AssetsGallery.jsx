...
import { ArrowLeft, ArrowUpRight, Check, Component, Cube, Download, MediaImage, MusicNote, Page, PauseSolid, PlaySolid, Search, Sparks, TextSize, Xmark } from 'iconoir-react';
...
const sections = [
  ...
  { id: 'interface', name: '界面里的小帮手', description: '当前应用实际使用的 Iconoir 图标，可直接下载 SVG。' },
...
function DownloadLink({ asset, label, className = '' }) {
  return <a className={`ag-download ${className}`} href={`${assetUrl(asset.path)}?v=${asset.sha256.slice(0, 12)}`} download={asset.path.split('/').at(-1)} aria-label={`下载${asset.name}`}><Download width={17} height={17} />{label || '下载原文件'}</a>;
}
...
function RiveCard({ asset }) {
  ...
  return <article className="ag-card ag-rive-card" ref={hostRef}>
    <div className={`ag-rive-stage ${asset.name === '绿豆' ? 'ag-rive-stage--green' : ''}`}>
      ...
      {!enabled && <button className="ag-rive-load" type="button" onClick={() => setEnabled(true)}><PlaySolid width={34} height={34} /><span>载入{asset.name}</span><small>{formatBytes(asset.bytes)} · Rive 原文件</small></button>}
    ...
    <div className="ag-card-body"><div className="ag-card-title"><h3>{asset.name}</h3><span className="ag-format-label">RIVE</span></div><AssetDetails asset={asset} />
      {status === 'ready' && choices.length > 0 && <div className="ag-rive-controls"><button type="button" onClick={toggle} aria-label={playing ? `暂停${asset.name}动画` : `播放${asset.name}动画`}>{playing ? <PauseSolid width={20} height={20} /> : <PlaySolid width={20} height={20} />}</button><select aria-label={`选择${asset.name}动画`} value={selection} onChange={(event) => choose(event.target.value)}>{choices.map((item) => <option value={item.value} key={item.value}>{item.name} · {item.kind}</option>)}</select><span>{choices.length} 项</span></div>}
      <div className="ag-card-footer"><DownloadLink asset={asset} />{asset.source && <a className="ag-source" href={asset.source} target="_blank" rel="noreferrer">{asset.sourceLabel}<ArrowUpRight width={13} height={13} /></a>}</div>
    </div>
  </article>;
}
function ImageCard({ asset }) {
  ...
  return <article className={`ag-card ag-image-card ${asset.category === 'image' ? 'ag-image-card--wide' : ''}`}>
    <a className="ag-image-stage" href={assetUrl(asset.path)} target="_blank" rel="noreferrer" aria-label={`查看${asset.name}原图`}>{error ? <MediaImage width={32} height={32} /> : <img src={assetUrl(asset.path)} alt={asset.name} loading="lazy" onError={() => setError(true)} />}</a>
...
function IconCard({ asset }) {
  const Icon = usedIcons[asset.iconName]; const iconRef = useRef(null);
  const weights = Array.isArray(asset.weights) && asset.weights.length ? [...asset.weights].map((value) => String(value)) : ['2.5'];
  const [stroke, setStroke] = useState(weights[0]);
  ...
  function download() {
    ...
    svg.setAttribute('xmlns', 'http://www.w3.org/2000/svg'); svg.setAttribute('width', '256'); svg.setAttribute('height', '256'); svg.setAttribute('fill', 'none');
    const url = URL.createObjectURL(new Blob([new XMLSerializer().serializeToString(svg)], { type: 'image/svg+xml' }));
    const link = document.createElement('a'); link.href = url; link.download = `${asset.iconName}-stroke-${stroke}.svg`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); setSaved(true);
  }
  return <article className="ag-card ag-interface-card"><div className="ag-interface-stage" ref={iconRef}>{Icon && <Icon width={42} height={42} strokeWidth={Number(stroke)} />}</div><div className="ag-card-body"><h3>{asset.name}</h3><p className="ag-icon-name">{asset.iconName}</p><div className="ag-icon-bottom">{weights.length > 1 ? <select value={stroke} aria-label={`${asset.name}线条粗细`} onChange={(event) => setStroke(event.target.value)}>{weights.map((value) => <option key={value}>{value}</option>)}</select> : <span>{stroke}</span>}<button className="ag-icon-download" type="button" onClick={download} aria-label={`下载${asset.name}SVG`}>{saved ? <Check width={18} height={18} /> : <Download width={18} height={18} />}</button></div></div></article>;
}
function AudioCard({ asset }) {
  return <article className="ag-card ag-audio-card"><div className="ag-audio-heading"><MusicNote width={23} height={23} /><div><h3>{asset.name}</h3><AssetDetails asset={asset} /></div></div><audio controls preload="none" src={assetUrl(asset.path)} aria-label={`${asset.name}试听`} onPlay={(event) => document.querySelectorAll('.ag-page audio').forEach((audio) => { if (audio !== event.currentTarget) audio.pause(); })} /><DownloadLink asset={asset} label="下载声音" /></article>;
}
function FileCard({ asset }) {
  return <article className={`ag-file-row ${asset.category === 'font' ? 'ag-file-row--font' : ''}`}><div className="ag-file-icon">{asset.category === 'font' ? <TextSize width={27} height={27} /> : <Page width={25} height={25} />}</div><div className="ag-file-info"><h3>{asset.name}</h3><p>{asset.path}</p><AssetDetails asset={asset} /></div><DownloadLink asset={asset} label="下载" /></article>;
}
...
export default function AssetsGallery() {
  ...
  return <main className="ag-page">
    <header className="ag-header"><a href={import.meta.env.BASE_URL} className="ag-brand"><img src={assetUrl('favicon-32.png')} width="28" height="28" alt="" /><span>赛博叫叫 <b>2027</b></span></a><a className="ag-back" href={import.meta.env.BASE_URL}><ArrowLeft width={16} height={16} />回到相机</a></header>
    <div className="ag-main"><section className="ag-intro" aria-labelledby="ag-title"><div className="ag-intro-label"><Component width={18} height={18} />叫叫的创作积木</div><h1 id="ag-title">资源陈列室<span className="ag-title-spark"><Sparks width={38} height={38} /></span></h1><p>会动的朋友，立体的美味，还有每一个小细节。<br className="ag-mobile-break" />在这里看看、玩玩，带走原文件。</p><div className="ag-summary" aria-live="polite"><span><Cube width={17} height={17} />{counts.model || 0} 个 3D 模型</span><span>{counts.rive || 0} 个 Rive 文件</span><span>{assets.length} 项资源</span></div></section>
      <div className="ag-toolbar"><nav className="ag-categories" aria-label="资源分类">{categoryGroups.map((item) => <button type="button" key={item.id} aria-pressed={category === item.id} className={category === item.id ? 'is-selected' : ''} onClick={() => setCategory(item.id)}>{item.name}<span>{counts[item.id] || 0}</span></button>)}</nav><div className="ag-search"><Search width={19} height={19} /><input aria-label="搜索资源名称或格式" placeholder="搜索名称、格式…" value={query} onChange={(event) => setQuery(event.target.value)} />{query && <button type="button" aria-label="清空搜索" onClick={() => setQuery('')}><Xmark width={15} height={15} /></button>}</div></div>
      {!catalog && !loadError && <div className="ag-empty" role="status"><Component width={38} height={38} /><h2>正在布置陈列室…</h2></div>}
      {loadError && <div className="ag-empty" role="alert"><Page width={38} height={38} /><h2>资源清单暂时无法打开</h2><button type="button" onClick={() => window.location.reload()}>重新加载</button></div>}
      {catalog && filtered.length === 0 && <div className="ag-empty" role="status"><Search width={38} height={38} /><h2>还没找到这个资源</h2><p>试试“苹果”“Rive”或“PNG”。</p><button type="button" onClick={() => { setQuery(''); setCategory('all'); }}>查看全部资源</button></div>}
      ...
      <footer className="ag-footer"><span>赛博叫叫 2027 · 资源陈列室</span><a href={assetUrl('asset-catalog.json')} download>下载完整资源清单<ArrowUpRight width={14} height={14} /></a></footer>
    </div>
  </main>;
}
