...
const iconNames = { ArrowSeparate: '切换镜头', Check: '确认', Download: '下载', Lock: '锁定', MediaImageList: '相册', MultiWindow: '画中画', NavArrowDown: '展开', OpenBook: '日志', PlaySolid: '播放', Refresh: '重新开始', Search: '搜索', Sparks: '灵感', ViewGrid: '全部', Xmark: '关闭' };
...
for (const file of await walk(publicRoot)) {
  ...
  assets.push({
    ...
    ...(relative === 'media/jiaojiao.riv' ? { sourceLabel: '叫叫 · 2026-09-18 斗篷修正版' } : {}),
  });
}
...
for (const file of await walk(path.join(root, 'src'))) {
  ...
  for (const match of source.matchAll(/import\s*\{([^}]+)\}\s*from\s*['"]iconoir-react['"]/g)) {
    for (const raw of match[1].split(',')) {
      const [name, alias] = raw.trim().split(/\s+as\s+/); if (!name) continue;
      const local = alias || name;
      if (local === 'IconoirProvider') continue;
      ...
      for (const usage of usages) current.weights.add(usage[1].match(/strokeWidth=\{\s*(\d+(?:\.\d+)?)\s*\}/)?.[1] || '2.5');
...
for (const icon of icons) assets.push({ id: `iconoir:${icon.name}`, name: iconNames[icon.name] || icon.name, iconName: icon.name, category: 'interface', format: 'SVG', weights: icon.weights, usedIn: icon.usedIn, license: 'MIT' });
const componentFile = `// Generated by scripts/generate-asset-manifest.mjs from actual application imports.\nimport { ${icons.map((icon) => icon.name).join(', ')} } from 'iconoir-react';\nexport const usedIcons = { ${icons.map((icon) => icon.name).join(', ')} };\n`;
