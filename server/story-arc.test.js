...
test("story arc provides 7 days for every age group", () => {
...
test("getStoryArc wraps days cyclically and falls back to mid", () => {
  assert.equal(getStoryArc(1, "low").day, 1);
  assert.equal(getStoryArc(7, "high").day, 7);
  assert.equal(getStoryArc(8, "mid").day, 1);
  assert.equal(getStoryArc(0, "mid").day, 7);
...
test("buildStoryInstructions includes premise, hook and closing guidance", () => {
  ...
  assert.ok(closing.includes("收尾：今天的故事已经讲了约三分钟"));
